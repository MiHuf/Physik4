...
\begin{align*}
  u(x) = u(R - R_0) = \left. u(x) \right|_{x = 0} +
    a \cdot \left.\frac{\mathrm d u(x)}{\mathrm d x} \right|_{x = 0} \cdot x
...
Ein Vergleich mit dem Potential $U(x) = - \int_{-\infty}^x F(x') dx'$ eines
Federpendels $F = - k \cdot x$, also $U(x) = \frac{1}{2} k x^2$ ergibt
mit \eqref{eq-DU} die Federkonstante:
\begin{align*}
